...
When deployment fails the [Rescue mode](intro-concepts.md#rescue-mode) action can be used to boot ephemerally into the node, followed by an investigation.
...
![web UI image list](../media/troulbeshoot-faq__2.3_images.png)
...
[note] `sudo` is required when extracting the image filesystem and when making changes to the files extracted from the image filesystem. [/note]
...
![web UI deploy image](../media/troulbeshoot-faq__2.3_deploy.png)
