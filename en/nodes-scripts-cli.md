The [web UI](nodes-scripts.md) enables you to easily write, upload and execute your own hardware testing scripts and see the results.
...
A [script's metadata](nodes-scripts.md#hardware-test-script-sample), and even the script itself, can be updated from the command line:
...
The JSON formatted output to the above command will include 'history' dictionary entries for when the script was modified along with any comments for those modifications:
...
You can also update whether or not results that have failed should be suppressed. This is useful in the event that you would like to ignore a known failure:
